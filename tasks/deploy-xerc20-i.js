...
let xERC20ImpAddr, lockboxImpAddr;

task("deploy-xerc20-i", "it deploys the set of XERC20 contracts")
  ...
  .setAction(async (taskArgs) => {
    ...
    const impsConfig = await readJson(`./tasks/templates/imps.json`);
    await initializeImplementations(impsConfig);
...
async function deployXERC20Imp() {
  ...
  let xERC20ImplAddress = xERC20ImpAddr;
...
async function deployLockboxImp() {
  ...
  let lockboxImplAddress = lockboxImpAddr;
...
async function deployXERC20(xERC20Config) {
  ...
  const xERC20ImplAddress = await deployXERC20Imp();
  ...
  tx = await proxy.initialize(xERC20ImplAddress, proxyAdminAddress, calldata);
  await tx.wait();
...
async function deployLockBox(xERC20Address, baseTokenAddress) {
  ...
  const lockboxImplAddress = await deployLockboxImp();
  const factory = await hre.ethers.getContractAt("BridgeFactory", factoryAddress);
  ...
  const proxy = ProxyFactory.attach(event.args.addr);
  tx = await proxy.initialize(lockboxImplAddress, proxyAdminAddress, calldata);
  await tx.wait();
...
const initializeImplementations = async (impsConfig) => {
    const localChainID = network.config.chainId;
    const data = impsConfig[localChainID];

    xERC20ImpAddr = data["xERC20ImpAddr"];
    lockboxImpAddr = data["lockboxImpAddr"];
