...
describe("Rebalancer, InETH, CrossChainBridge, Lockbox, and LiquidPool Contracts", function () {
    async function deployContractsFixture() {
        ...
        const CrossChainBridgeFactory = await hre.ethers.getContractFactory("CrossChainBridge");
        const crossChainBridge = await CrossChainBridgeFactory.deploy(owner.address, liquidPoolAddress);
        const crossChainBridgeAddress = await crossChainBridge.getAddress();

        // Set CrossChainBridge address in Rebalancer
        await rebalancer.setCrossChainBridge(crossChainBridgeAddress);

        // Set Rebalancer address in CrossChainBridge
        await crossChainBridge.setRebalancer(rebalancerAddress);
        ...
        return { inETH, rebalancer, crossChainBridge, lockbox, liquidPool, owner };
    }
    ...
    describe("updateTreasuryData() Function", function () {
        it("Should update treasury data when L1 ratio - L2 ratio is lower than MAX_DIFF", async function () {
            const { inETH, rebalancer, crossChainBridge, lockbox, liquidPool } = await loadFixture(deployContractsFixture);
            ...
            await crossChainBridge.receiveL2InfoOptimism(currentTimestamp, l2EthAmount, l2InEthAmount);
        ...
        it("Should update treasury data when L1 ratio - L2 ratio is higher than MAX_DIFF", async function () {
            const { rebalancer, crossChainBridge, liquidPool, inETH } = await loadFixture(deployContractsFixture);
        ...
        it("Should correctly mint tokens when expected", async function () {
            const { rebalancer, inETH, crossChainBridge, lockbox, liquidPool } = await loadFixture(deployContractsFixture);
